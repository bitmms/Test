...
namespace MyMVVM.SoftReg.Utils
{
    public class SoftRegUtil
    {
        ...
        public string GetMNum()
        {
            ...
            string cpuNum = GetCpu().Substring(0, 8);
            string diskNum = GetDiskVolumeSerialNumber().Substring(GetDiskVolumeSerialNumber().Length - 4, 4);
            string macNum = GetMacByNetworkInterface().Replace("-", "").Substring(0, 12);

            string strNum = cpuNum + diskNum + macNum;
